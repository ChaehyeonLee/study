...
def bfs(tree, root, target):
    visited = [] #방문한 node들이 순서대로 visited리스트에 들어간다. bfs한 결과를 나타냄
    queue = [root] #방문할 node들이 queue에 저장된다.
    ...
    while queue:
        ...
        if node not in visited:
            ...
            if node == target: #target node를 찾으면 탐색을 중지함
                return visited #root node부터 target node까지의 bfs 결과를 반환함
            neighbor = tree.get(node) #현재 node의 neighbor node를 받아옴
            if neighbor:
                queue.extend(neighbor)
    return visited #root node부터 tree 전체를 bfs한 결과를 반환함
...
print('test2_output:', bfs(test_tree, 'B', 'C')) #'B'를 root node로 test_tree에서 'C'를 찾을 때 까지 bfs 
