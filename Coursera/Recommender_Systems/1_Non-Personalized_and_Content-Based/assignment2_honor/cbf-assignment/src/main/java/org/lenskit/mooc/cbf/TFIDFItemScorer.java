    ...
    public ResultMap scoreWithDetails(long user, @Nonnull Collection<Long> items){
        ...
        for (Long item: items) {
            ...
            for (Map.Entry<String, Double> entry : userVector.entrySet()) {
                uvLen += Math.pow(entry.getValue(), 2);
            }
            uvLen = Math.sqrt(uvLen);

            double ivLen = 0.0;
            for (Map.Entry<String, Double> entry : itemVector.entrySet()) {
                ivLen += Math.pow(entry.getValue(), 2);
            }
            ivLen = Math.sqrt(ivLen);

            double denominator = uvLen * ivLen;

            double dotProduct = 0.0;
            for (String attribute: userVector.keySet()) {
                if (itemVector.containsKey(attribute)) {
                    dotProduct += userVector.get(attribute) * itemVector.get(attribute);
                }
            }
            ...
            if (denominator != 0) {
                cosSimilarity = dotProduct / denominator;
            }
            // TODO And remove this exception to say you've implemented it --> removed

            results.add(Results.create(item, cosSimilarity));
        }
